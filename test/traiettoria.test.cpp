...
TEST_CASE("Test del calcolo delle traiettorie"){
    ...
    SUBCASE("Test calcolo orientazione traiettoria"){
        CHECK(biliardo.getHitDirection(p0.theta)==particleSimulator::vecOrientation::Up);
        CHECK(biliardo.getHitDirection(p1.theta)==particleSimulator::vecOrientation::Down);
        CHECK(biliardo.getHitDirection(p2.theta)==particleSimulator::vecOrientation::HorizontalRight);
        CHECK(biliardo.getHitDirection(p3.theta)==particleSimulator::vecOrientation::HorizontalLeft);
