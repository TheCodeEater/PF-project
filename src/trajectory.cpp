...
// costruttore oggetto di calcolo traiettorie
...
intsect path::operator()(particle const& p) const {
  ...
  assert(getLocationType(p.pos) == posTypes::Inside ||
         std::abs(p.pos.x()) <
             constants::eps());  // devi essere dentro il biliardo oppure
                                 // sull'asse Y
  ...
  const intsect intersec = [this, &trajectory, &p]() -> intsect {
    vecOrientation orientation{
        getHitDirection(p.theta)};  // orientazione del vettore
    ...
    switch (orientation) {
      case vecOrientation::Right: {  // direzione destra
        ...
        if (exit_intsec.y() < r2_ + constants::eps() &&
            exit_intsec.y() > -r2_ - constants::eps()) {
  ...
  assert(getLocationType(intersec.point) !=
         posTypes::Error);  // assicurati che sia possibile determinare il tipo
                            // di posizione in cui si trova
...
void path::reflect(particle& p) const {
  ...
  switch (intsect.border) {
    case hitBorder::Front:  // colpito pseudobordo frontale: memorizza punto di
                            // uscita
      p.pos = intsect.point;
      break;

    case hitBorder::Back:  // colpito pseudobordo posteriore: memorizza punto di
                           // uscita
      p.pos = intsect.point;
      break;
...
void path::rotate(particle& p, Eigen::Vector2f const& normal_vect,
  ...
  const Line normal{intersection.point,
                    normal_vect};  // trova la retta normale al bordo per il
  ...
  // NOTA: phi non deve rispettare la convenzione sugli angoli, il segno
  // determina il verso della rotazione
  ...
  const float new_angle = arctan(
      new_dir.y(), new_dir.x());  // determina l'angolo della nuova traiettoria
...
posTypes path::getLocationType(Eigen::Vector2f const& v)
  ...
  if (v.x() > constants::eps() &&
      v.x() <=
          l_ - constants::eps()) {  // coordinata x entro i limiti del biliardo
...
vecOrientation path::getHitDirection(float const& angle) const {
  ...
  } else if (angle > constants::eps() &&
             angle < bm::pi<float>() / 2 - constants::eps()) {  // alto a dx
    return vecOrientation::Right;
...
bool path::testOutConditions(particle const& p) const {
  // verifica che, in caso in cui la particella sia uscita, la coordinata Y sia
  // corretta
  if (getLocationType(p.pos) ==
      posTypes::BackHit) {  // se esci da dietro, testa con r1
    return p.pos.y() <= r1_ + constants::eps() &&
           p.pos.y() >= -r1_ - constants::eps();
  } else if (getLocationType(p.pos) ==
             posTypes::Escaped) {  // da davanti con r2
    return p.pos.y() <= r2_ + constants::eps() &&
           p.pos.y() >= -r2_ - constants::eps();
  } else {  // si ha se la particella non esce oppure getLocationType
            // restituisce errore
    return false;
  }
}
...
results simulation::operator()(particle& p) const  // operatore di simulazione
{
