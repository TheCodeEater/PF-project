...
struct particle {
  ...
  float theta{};  // angolo traiettoria, tra 0 e 2pi, misurato dall'asse X in
                  // senso antiorario
};
struct exit_point {  // punto di uscita, con angolo di traiettoria di uscita
  ...
  exit_point operator-=(T const& rhs) {
    static_assert(
        std::is_arithmetic_v<T>);  // verifica che sia un valore numerico
    y -= rhs;
    theta -= rhs;
...
exit_point operator*(exit_point const& lhs, T const& rhs) {
  static_assert(std::is_arithmetic_v<T>);  // verifica che sia un valore
                                           // numerico
